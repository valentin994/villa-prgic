    ...
    return(    
        <nav className="p-3 border-gray-200 rounded bg-gray-50">
          <div className="container flex flex-wrap items-center justify-between mx-auto">
            ...
            <div className={`w-full md:block md:w-auto ${isMenuOpen ? "opacity-100": "opacity-0"} ${isMenuOpen ? "max-h-full": "max-h-0"}`} id="navbar-solid-bg">
              <ul className="flex flex-col mt-4 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-transparent">
                ...
                <li>
                    <Link to="/apartments" onClick={() => setIsMenuOpen(false)} className="block py-2 pl-3 pr-4 rounded md:bg-transparent md:text-blue-700 md:p-0">Contact</Link>
                </li>
              </ul>
            </div>
          </div>
